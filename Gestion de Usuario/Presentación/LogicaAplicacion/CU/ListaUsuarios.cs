...
namespace LogicaAplicacion.CU
{
    public class ListaUsuarios : IListadoUsuarios
    {
        ...
        public IEnumerable<ListadoUsuariosDTO> DTOUsuarios()
        {
            IEnumerable<Usuario> user = RepositorioUsuario.FindAll();
            foreach (Usuario usuario in user)
            {
                usuario.IdRol = usuario.Rol.Id;
                usuario.IdsTelefonos = usuario.Telefonos.Select(t => t.Id).ToList();
                usuario.Telefonos.Select(t => t.IdUsuario = usuario.Id).ToList();
            }
